...
@Component({
  selector: "app-rebackbankcheck",
  templateUrl: "reback_bank_check.component.html",
})
export class RebackBankCheckComponent implements OnInit {
  ...
  getallchecks() {
    var res;
    this.admin.active = true;
    this.bankservices.getallchecks("reback").subscribe({
      next: (data) => {
        res = data;
        if (res.code == 100) {
          this.admin.active = false;
