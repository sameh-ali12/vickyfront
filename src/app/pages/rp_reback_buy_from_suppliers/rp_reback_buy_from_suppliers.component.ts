...
@Component({
  selector: "app-rprebackbuyfromsuppliers",
  templateUrl: "rp_reback_buy_from_suppliers.component.html",
})
export class RpReBackBuyFromSuppliersComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.buyFromsuppliersservice.getrebackbuybillswithsupplieranddate(this.paymentsuppliersForm.value.supplier,this.paymentsuppliersForm.value.from,this.paymentsuppliersForm.value.to).subscribe({
      next: (data) => {
        res = data;
        if (res.code == 100) {
